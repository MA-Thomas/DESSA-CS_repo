...
minPropensityDuration = 1e-6; % units [s] 
...
% ipptime = tic;
% if trial == 1
% [Integrated_CDFs,distances,numVariances,var_list] = preCompute_propensityCDF_curves(...
%     contact_sphere_radiusSqd,T_HARDLIMIT_DIFFUSE,maxRxDist_T_HARD,Da,Db,...
%     earliestPropensityTime);
% [Integrated_CDFs,distances,numVariances,var_list] = Integrated_prePropensities_pointParticle(...
%     contact_sphere_radiusSqd,T_HARDLIMIT_DIFFUSE,maxRxDist_T_HARD,Da,Db,...
%     earliestPropensityTime);
% end
...
[ PQueue,initial_events,lists_cell ] = initializeRxQueue( assemblyDistMatrix,...
    currentAssemblyIDs,...
...
t_absolute
display(["max_d_index is ",num2str(max_d_index)]) % TEMP ----
