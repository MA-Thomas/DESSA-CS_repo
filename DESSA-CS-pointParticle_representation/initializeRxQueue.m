function [ PQueue,initial_events,lists_cell ] = initializeRxQueue( assemblyDistMatrix,...
    currentAssemblyIDs,...
    PQueue,t_absolute,assemblies_lastUpdate_absTime,allAssembliesXYZ,...
    maxRxDistance,n_sigma,rate_constant,Dmonomer,...
    containerLength,currentAssemblyTYPES,...
    exceedBoundaryDist,minPropensityDuration,T_HARDLIMIT_DIFFUSE,...
    var_list,numVariances,distances,Integrated_CDFs)
...
for row = 1:size(assemblyDistMatrix,1)
    ...
    %disp(["col_indices2 = ",num2str(col_indices2)])
    ...
    if d < inf % This entry is a valid reaction pair.
        ...
        if d_index > max_d_index; max_d_index = d_index; end % TEMP -----------
        assert(t_offset_a==0)
        ...
        % duration.
        Pk = log( 1/rand );     
        if rate_constant*Integrated_CDFs(d_index,end) < Pk
            % No reaction in allowed max diffusion time
            wait_time = -1;

        else
            IntegratedPropensity_curve = rate_constant * Integrated_CDFs(d_index,:);
            wait_time = computeWaitTime_preComp_CDF(IntegratedPropensity_curve, var_list,...
                Da, Db, t_offset_b, Pk);    
        end
%         wait_time = computeWaitTime_preComp_CDF(IntegratedPropensity_curve, var_list,...
%             Da, Db, t_offset_b);
...
display(["max_d_index is ",num2str(max_d_index)])
end
