...
func main() {
	...
	//ratioList := []float64{100} //[]float64{0.1, 1, 100} // kakDRatio float64
	...
	// the boundary parameter is used to compute the PDF (in the finite particle size representation)
	...
	dmax := 90.9 //2 * n_sigma * math.Sqrt(6*Dmonomer*T_hardlimit_diffuse)
	...
	numTimes5 := 1e2
	numTimes6 := 1e2
	totalNumTimes := int(numTimes1 + numTimes2 + numTimes3 + numTimes4 + numTimes5 + numTimes6)
	...
	shortTime4 := 1.0
	shortTime5 := 2.0
	timeSpacing1 := (shortTime1 - tmin) / float64(numTimes1)
	timeSpacing2 := (shortTime2 - shortTime1) / float64(numTimes2)
	timeSpacing3 := (shortTime3 - shortTime2) / float64(numTimes3)
	timeSpacing4 := (shortTime4 - shortTime3) / float64(numTimes4)
	timeSpacing5 := (shortTime5 - shortTime4) / float64(numTimes5)
	timeSpacing6 := (tmax - shortTime5) / float64(numTimes6)

	times_list := make([]float64, totalNumTimes)
	times_list[0] = tmin
	for i := 1; i < totalNumTimes; i++ {
		if times_list[i-1]+timeSpacing1 < shortTime1 {
			times_list[i] = times_list[i-1] + timeSpacing1

		} else if times_list[i-1]+timeSpacing2 < shortTime2 {
			times_list[i] = times_list[i-1] + timeSpacing2
		} else if times_list[i-1]+timeSpacing3 < shortTime3 {
			times_list[i] = times_list[i-1] + timeSpacing3
		} else if times_list[i-1]+timeSpacing4 < shortTime4 {
			times_list[i] = times_list[i-1] + timeSpacing4
		} else if times_list[i-1]+timeSpacing5 < shortTime5 {
			times_list[i] = times_list[i-1] + timeSpacing5
		} else {
			times_list[i] = times_list[i-1] + timeSpacing6
		}
	}
	// -----------------------------
	...
	/*
		...
		startTime = time.Now()
		BinValues, BinTimes := rebindDistrib(Integrated_Propensities[0], times_list)
	...
	*/
	...
	//fileName := "integratedPropensities_FINITE_ka_div_kD__"+fmt.Sprintf("%e", kakDRatio)+".csv"
	...
	//fileName = "PDFs_FINITE_ka_div_kD__"+fmt.Sprintf("%e", kakDRatio)+".csv"
	...
	*/
	...
	/*
		//fileName = "waitTimes_rebindingDistribution_ka_div_kD__1e-1.csv"
		//success = writeCSV2(waitingTimes, fileName)
		//fmt.Println(success)

		fileName = "BinValues_rebindingDistribution_ka_div_kD__"+fmt.Sprintf("%e", kakDRatio)+".csv"
		//fileName = "BinValues_rebindingDistribution_ka_div_kD__1e-1.csv"
		success = writeCSV2(BinValues, fileName)
		fmt.Println(success)

		fileName = "BinTimes_rebindingDistribution_ka_div_kD__"+fmt.Sprintf("%e", kakDRatio)+".csv"
		//fileName = "BinTimes_rebindingDistribution_ka_div_kD__1e-1.csv"
		success = writeCSV2(BinTimes, fileName)
		fmt.Println(success)
	*/
...
func evalIntegralN(r0, contactR, D, c float64, times_list []float64, numTimes, d_ind int, Ch chan integrationOutput) {
	...
	for t_ind := 0; t_ind < numTimes; t_ind++ {
		...
		//fmt.Println("term1, term2, term3, term4, summand: ",term1,term2,term3,term4,summand)
		//fmt.Println("a,b are: ",math.Pow(B, 2)*D*tau+B*(r0-contactR),(r0-contactR)/(2*math.Sqrt(D*tau))+B*math.Sqrt(D*tau))
		//fmt.Println("result / time: ",result, tau)
		//fmt.Println("")
		//fmt.Println("summand: ",summand)
		///*
